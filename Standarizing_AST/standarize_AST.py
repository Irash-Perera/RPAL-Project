...
class AST:
    ...
    def standardize_node(self, node):
        ...
        if node_type == ASTNodeType.LET:
            ...
            #iterate through all EQUAL nodes
            while child_node:
            ...
            x_with_sibling_e = child_node.get_child()
            lambda_node = ASTNode()
            lambda_node.set_type(ASTNodeType.LAMBDA)
            lambda_node.set_child(x_with_sibling_e) 
            ...
            x = ASTNode()
            x.set_child(x_with_sibling_e.get_child())
            x.set_sibling(gamma_node)
            x.set_type(x_with_sibling_e.get_type())
            x.set_value(x_with_sibling_e.get_value())
            node.set_child(x)
            node.set_type(ASTNodeType.EQUAL)
